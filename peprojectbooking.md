...
User data includes:
o	username: Login name (String).
o	password: Encrypted password (String).
o	role: User role (Enum: "admin" or "student").
o	createdAt: Creation date (Date).
...
•	Requirements:
o	Students can only register/unregister for events.
o	Admins can manage event registrations (view/search the list).
•	Implementation:
o	Develop a middleware to verify the JWT Token and enforce role-based access control.
...
o	studentId: Student ID (String).
o	eventId: Event ID (String).
o	registrationDate: Registration timestamp (Date).
